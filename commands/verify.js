  ...
  async execute(interaction) {
    ...
    if (!interaction.channel.name.match(/verification-\d*/)) {
      await interaction.reply(
        "This is not a valid channel to use this command in!",
      );
      return;
    }
      ...
      .then(async (messages) => {
        ...
        messageContents.forEach(async (message) => {
          var embed = new EmbedBuilder()
            ...
            .setFooter({
              text: `Verified by: ${interaction.member.user.username}`,
            ...
            .setTimestamp();
          await interaction.client.channels.cache.get(verifyChannel).send({
            embeds: [embed],
          });
        });

        interaction.guild.members.cache.get(target.id).roles.add(
          serverMemberRole,
        );
        ...
        var welcomeMessage =
          `Welcome ${target.toString()} to ${interaction.guild.name}! We're happy to have you here :sparkles:\n\n` +
          ...
          `(<@&${welcomeRole}>)\n`;
        await interaction.client.channels.cache.get(generalChannel).send(
          welcomeMessage,
        );
